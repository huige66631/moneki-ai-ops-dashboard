...
from app.ai.contracts import AssistantResponse, AskRequest, Evidence, Navigation, ProviderInfo, ToolCall, ToolCallPlan
...
from app.ai.turn_resolver import resolve_turn
...
def ask(
    ...
    incoming_session_id = str(request.session_id) if request.session_id else None
    previous_state = store.touch(incoming_session_id) if incoming_session_id else None
    session = store.get_or_create(incoming_session_id)
    plan = resolve_turn(request.question, request.context, previous_state, bounds)
    if plan is None:
        try:
            plan = provider.plan(request.question, (request.context.start_date, request.context.end_date) if request.context else None, bounds)
        except ProviderError:
            raise
    provider_info = ProviderInfo(name=provider.name, mode=provider.mode)
    if plan.status != "tool_call":
        return AssistantResponse(status=plan.status, answer=plan.message or "当前问题需要更多信息。", intent=plan.intent, session_id=session.session_id, provider=provider_info)
    ...
    try:
        ...
        if plan.name == "get_category_store_revenue":
            ...
            if not result["winner"]:
                return AssistantResponse(status="needs_clarification", answer="这个日期范围没有可用的销售记录，请换一个日期范围。", intent="category_store_revenue", session_id=session.session_id, provider=provider_info)
            ...
            if len(matches) != 1:
                ...
                return AssistantResponse(status="needs_clarification", answer=f"我没有找到唯一的商品“{product_name}”（{detail}），请提供 products 表中的准确商品名称。", intent="product_revenue", session_id=session.session_id, provider=provider_info)
            ...
            if result["direction"] == "insufficient_data":
                return AssistantResponse(status="needs_clarification", answer="当前范围不足以比较两个完整的 7 天客单价窗口，或其中一个窗口没有订单。请扩大日期范围。", intent="recent_average_order_value", session_id=session.session_id, provider=provider_info)
        ...
        arguments = {**plan.arguments, "start_date": start_date, "end_date": end_date}
        store.record_answered(session.session_id, intent, plan.name, arguments)
        return AssistantResponse(
            status="answered",
            answer=answer,
            intent=intent,
            session_id=session.session_id,
            tool_call=ToolCall(name=plan.name, arguments=arguments),
            evidence=evidence,
            navigation=Navigation(start_date=start_date, end_date=end_date),
